...
//Класс для разреженной матрицы: проверки ее пустоты и ее задания
//наслудется от интерфейса с аналогичной задачей (для объектов класса Figure)
class FigureMatrixCheckEmpty : IMatrixCheckEmpty<Figure>
{
    // В качестве пустого элемента возвращается null (формально, пустота)
...
public class SimpleListItem<T>
{
    ...
    {
        //Задание непосредственно данных элемента стэка
        this.data = param;
    }
}
// Список
public class SimpleList<T> : IEnumerable<T> where T : IComparable
{
    ...
    public SimpleListItem<T> GetItem(int number)
    {
        ...
        {
            throw new Exception("Выход за границу списка");
        }
        //Подставляем на место локального элемента первый элемент переданного стэка
